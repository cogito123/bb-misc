...
function create_description()
  ...
  local texts = {
        ...
        'Biters gain movement and attack speed with match time',
        'a',
        'b'
  }
...
local adjust_unit_speed = [[function(event)
  ...
  local factor = math.max(1, minutes / 60)
...
local on_tick = [[function(event)
  ...
  local ticks = functions.get_ticks_since_game_start()
  local minutes = ticks / 3600
  local melee_spd = minutes / 60
  local biological_spd = minutes / 60
  local forces = {
    "north_biters",
    "north_biters_boss",
    "south_biters",
  ...
  local mov_spd = math.max(1, minutes / 60)
  storage.cbs['info'][3].text = string.format('Current movement speed at %.2f%%', mov_spd * 100)
  storage.cbs['info'][4].text = string.format('Current attack speed at %.2f%%', (melee_spd + 1) * 100)
end]]
